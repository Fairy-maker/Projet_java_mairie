...
\title{Rapport du projet de Java : Gestion des budgets d'une ville}
\date{}
...
\begin{enumerate}
	...
	\item Le package main : permet d'executer une résolution de problème de sac à dos multidimensionnel de manière interactive. 
\end{enumerate}
...
Cette classe fille héritée de la classe Personne a pour objectif de créer nos évaluateurs qui, rattachés à un type d’évaluation du coût, ont les compétences pour déterminer le coût d’un projet.
...
SacADosInterface et ObjetInterface sont implémentées dans le package sacADos.
...
Cette méthode calcule la somme des coûts indice par indice auxquels on retranche les budgets associés. Elle va ensuite chercher les indices où on retrouve la valeur maximale de ces résultats. Cela nous permet de paramétrer notre variable indicesMaxValue.

\subsubsection{maxDesCouts}
Cherche le maximum courant parmi les coûts d’un objet. Il renvoie le coût maximal. 

\subsubsection{interet}
Permet enfin de calculer l'intérêt d’un objet selon la formule de l’énoncé (grâce aux deux méthodes definies précédemment). Elle renvoie le résultat de l'intérêt de l’objet.
...
Il est important de noter que chaque réponse donnée par l’IA a été résumée ici. Nous avons résumé les points donnés par l’IA qui nous semblaient importants de mentionner et qui nous ont été utiles dans la procédure de notre projet. Une version non résumé à l’essentiel nous aurait amené à faire des captures d'écran très importantes pour des affirmations portées par l’IA qui ne nous étaient d’aucunes utilités.
\\
...
Ensuite, quand tu appelles evaluer(projet), l’évaluateur met à jour uniquement son type de coût.   
 ...
 =$>$ PS : j’avais déjà créé l’énumeration grace à une video youtube !
\\
...
\textbf{Critique} : Tout comme le test pour GloutonRetraitSolveur, j’ai gardé ce qu’il m’avait donné et l’ai un peu adapté en fonction de ce qu’on avait déjà.
\\
...
\begin{figure}[h]
    \centering
\includegraphics[width=0.6\textwidth]{reponsePromt5}
\end{figure}
...
\textbf{Réponse} : voici un lien vers la réponse \href{https://chat.mistral.ai/chat/92226b30-88fe-4807-8545-dee874162ad7}{https://chat.mistral.ai/chat/92226b30-88fe-4807-8545-dee874162ad7}.
...
La partie qui m’a posé le plus de difficulté a été la classe VersSacADos. Il a fallu réussir à transformer des projets municipaux en données exploitables par le sac à dos (tableau de coûts, de budgets et de bénéfices). 

De plus, cette transformation était aussi dépendante du scénario choisi, ce qui oblige à faire attention aux indices des tableaux et à faire correspondre les énumérations et les dimensions du sac (puisque selon le scénario, les coûts d’un projet ne sont pas répartis de la même manière). 
...
\begin{itemize}
	...
	\item Tous les TPs de Java Objet de cette année
...
\section{Lien ver le GitHub}
Voici le lien pour accéder à notre GitHub : \href{https://github.com/Fairy-maker/Projet_java_mairie.git}{https://github.com/Fairy-maker/Projet\_java\_mairie.git}
...
\begin{figure}[h]
    \centering
\includegraphics[width=0.6\textwidth]{modeleULM1}
\end{figure}

\begin{figure}[h]
    \centering
\includegraphics[width=0.6\textwidth]{modeleULM2}
\end{figure}

\begin{figure}[h]
    \centering
\includegraphics[width=0.6\textwidth]{modeleULM3}
\end{figure}
